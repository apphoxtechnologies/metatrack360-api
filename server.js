...
// --- UPDATED to use an environment variable for the frontend URL ---
const setPasswordUrl = `${process.env.FRONTEND_URL}/set-password?token=${resetToken}`; 
const mailOptions = { from: `"APPHOX MetaTrack360" <${process.env.EMAIL_USER}>`, to: email, subject: 'Welcome to APPHOX MetaTrack360 - Set Your Password', html: `<h1>Welcome, ${name}!</h1><p>An account has been created for you. Please click the link below to set your password.</p><a href="${setPasswordUrl}">Set Your Password</a><p>This link will expire in one hour.</p>`, }; await transporter.sendMail(mailOptions); res.status(201).json({ message: 'User created successfully! A setup email has been sent.'}); } catch (err) { console.error('Error creating user:', err); if (err.code === 'ER_DUP_ENTRY') { return res.status(400).json({ error: 'Email address already exists.' }); } res.status(500).json({ error: 'Failed to create user' }); } });
